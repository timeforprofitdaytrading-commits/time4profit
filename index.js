...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // Contador de visitas de la web: crea la tabla la primera vez (si no existe)
    // y suma 1 cada vez que alguien visita la web.
    if (url.pathname === "/api/visit") {
      await env.DB.prepare(
        "CREATE TABLE IF NOT EXISTS site_visits (id INTEGER PRIMARY KEY, count INTEGER NOT NULL DEFAULT 0)"
      ).run();
      await env.DB.prepare(
        "INSERT INTO site_visits (id, count) VALUES (1, 1) ON CONFLICT(id) DO UPDATE SET count = count + 1"
      ).run();
      const row = await env.DB.prepare("SELECT count FROM site_visits WHERE id = 1").first();
      return new Response(JSON.stringify({ value: row ? row.count : 1 }), {
        headers: { "content-type": "application/json", ...corsHeaders },
      });
    }
